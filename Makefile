BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME =Die_Eroberung_des_Suedpols_Vol_I
TITLE_NAME = "Die Eroberung des Suedpols Erster Band"
...
$(BUILD)/docx/$(OUTPUT_FILENAME).docx: $(MAKEFILE) $(METADATA) $(CHAPTERS) $(CSS_FILE) $(CSS_FILE_KINDLE) $(IMAGES) \
	...
	mkdir -p $(BUILD)/docx
	pandoc $(ARGS) --from markdown+raw_html+fenced_divs+fenced_code_attributes+bracketed_spans --to docx --resource-path=$(IMAGES_FOLDER) -o $@  $(METADATA_PDF)  $(PREFACE_HTML_PDF) $(CHAPTERS)
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA) $(CHAPTERS) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_EPUB)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	cp  $(IMAGES_FOLDER)/Die_Eroberung_des_Suedpols_Vol_I_*.jpg .
	cp  $(IMAGES_FOLDER)/cover*.jpg .
	cp  $(IMAGES_FOLDER)/logo.jpg .
	cp  $(IMAGES_FOLDER)/Roald_Amundsen.jpg .
	...
	rm  $(IMAGES_FOLDER)/*.css
	rm Die_Eroberung_des_Suedpols_Vol_I_*.jpg
	rm  cover*.jpg 
	rm logo.jpg
	rm Roald_Amundsen.jpg
